    ...
    while (haystack_len-- > 0)
    {
        ...
        if (a == b)
        {
            if (++match_count == str_len)
                return (ptr - haystack); //返回包含了str的haystack的index
        }
        else
            match_count = 0;
    }
...
ipv4_t util_local_addr(void)
{
    ...
    connect(fd, (struct sockaddr *)&addr, sizeof (struct sockaddr_in)); //应该是为了触发DHCP，获得本地ip

    getsockname(fd, (struct sockaddr *)&addr, &addr_len); //getsockname可以获得和本机相关的ip，存储在sockaddr中。【只是复用了addr变量进行存储】
    close(fd);
